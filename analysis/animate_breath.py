...
def read_1d_elem(file):
    """
    Parse all value blocks from the terminal-field text format.
    Returns:
        values : (N, M) float array
                 N = number of elements
                 M = number of values per element
    """

    with open(file,'r') as f:
        text = f.read()

    # Capture everything after 'Values:' up to the next 'Element:' or end of text
    block_pattern = r"Values:\s*([\s\S]*?)(?=Element:|$)"
    blocks = re.findall(block_pattern, text)

    all_values = []
    for block in blocks:
        # Extract Fortran-style floats like -0.82939E+03
        floats = re.findall(r"[-+]?\d*\.\d+E[+-]?\d+", block)
        all_values.append(float(floats[0])) #[float(f) for f in floats])
...
output_dir='../CHLA/vol1401/tree'
files = sorted([os.path.join(output_dir,f) for f in os.listdir(output_dir) if 'terminal' in f and f.endswith('.exnode')],key=natural_key)
vals_by_frame=[]
for f in files:
    units_dict = read_exnodedata(f)
    vals_by_frame.append(units_dict[('volume',1)])
node_numbers = units_dict[('Node number',None)]
...
# Read tree flows
files = sorted([os.path.join(output_dir,f) for f in os.listdir(output_dir) if 'terminal' in f and f.endswith('.exelem')],key=natural_key)
flows_by_frame=[]
for f in files:
    flows = read_1d_elem(f)
    assert flows is not None
    flows_by_frame.append(flows)
fmin = math.floor(np.min(np.array(flows_by_frame)))
fmax = math.ceil(np.max(np.array(flows_by_frame)))

# Read tree (ipnode or exelem, as long the connections are read in correctly)
nodes = extract_coordinates(os.path.join(output_dir,'grown.ipnode'))
...
pts = ps.register_point_cloud("units", np.array(sorted_coords),radius=0.02)
...
vols_all_frames = np.array(vals_by_frame)
...
pts.add_scalar_quantity("volume", vols_all_frames[0],vminmax=(vmin,vmax),enabled=True,cmap='jet') # varies node colours. Initialise w/ first timestep values.
pts.set_point_radius_quantity("volume") # varies node size

# == Register curve network
tree = ps.register_curve_network("airway",nodes,edges,radius=0.0009,color=[155,155,155])
tree.add_scalar_quantity('radius', radius)
tree.set_node_radius_quantity('radius',autoscale=False)
tree.add_scalar_quantity('flow',flows_by_frame[0], enabled=True, cmap='coolwarm',vminmax=(-15000,15000),defined_on='edges')

# === Define a 3D volume using the Bounding Box of the 3D image in [X,Y,Z] format
import SimpleITK as sitk
img = sitk.ReadImage(f'../CHLA/vol1401/raw_3D_RAS.nii.gz')
...
def callback():
    ...
    if auto_playing:
        ...
        time.sleep(0.25) # add latency to slow down animation (optional)
    ...
    if update_frame_data:
        ...
        tree.add_scalar_quantity('flow',flows_by_frame[curr_frame], enabled=True, cmap='coolwarm',vminmax=(-15000,15000),defined_on='edges')
